...
The two classes that use each other are termed as ‘coupled’. The class that needs other class in order to work are called dependants. The other called dependency. A dependant depends on its dependencies. Dependencies are bad because it will reduce the reusability of the class, make it hard for testing, and hard to maintain.

Dependency injections is a technique whereby one object supplies the dependencies of another object. Injecting dependencies to a dependent object that would use it.

Dagger 2 is one of the open-source DI frameworks which generates a lot of boilerplate for us. Dagger use annotation to produce a boilerplate code in compile runtime.

**3 Major Parts of Dagger2 :**

1. Dependency Provider

Is the one who provides the objects that are called dependencies. The class that is responsible for providing dependencies is annotated as @Module and the methods that provide dependency in this class to be annotated as @Provides.

2. Dependency Consumer

Dependency consumer is a class where we need to instantiate the objects. Dagger will provide the dependency if we annotated the object declaration with @Inject

3. Component

Acts as interface between dependency consumer and dependency provide annotated with @Component and it is an interface
